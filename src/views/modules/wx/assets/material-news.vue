<template>
    <div class="panel">
                    ...
                    <template v-for="(item,i) in dataList">
                        <div class="card" :key="item.mediaId" v-if="i%rows==n-1" @click="onSelect(item)">
                            <div class="card-preview">
                                <a v-for="(article,k) in item.content.articles" :key="k" :href="article.url"
                                   class="article-item" target="_blank">
                            ...
                            <div class="card-footer">
                                ...
                                <div class="flex justify-between align-center" v-if="!selectMode">
                                    <el-button size="mini" type="text" icon="el-icon-copy-document"
                                               v-clipboard:copy="item.mediaId" v-clipboard:success="onCopySuccess"
                                               v-clipboard:error="onCopyError">复制id
                                    </el-button>
                                    <el-button size="mini" type="text" icon="el-icon-edit"
                                               @click="addOrUpdateHandle(item)">编辑
                                    </el-button>
                                    <el-button size="mini" type="text" icon="el-icon-delete"
                                               @click="deleteHandle(item.mediaId)">删除
                                    </el-button>
                                    <el-button size="mini" type="text" icon="el-icon-finished"
                                               @click="publish(item.mediaId)">发表
                                    </el-button>
                                </div>
                            </div>
                        </div>
        ...
        <!-- 新增 / 修改 -->
        <add-or-update :visible="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="onChange"
                       @hide="addOrUpdateVisible=false"></add-or-update>
    </div>
</template>
<script>
    export default {
        ...
        methods: {
            ...
            publish(mediaId) {
                this.$confirm('发表内容', '确认', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(_ => {
                        this.$http({
                            url: this.$http.adornUrl('/manage/wxAssets/publish'),
                            method: 'post',
                            data: mediaId
                        }).then(({data}) => {
                            if (data && data.code === 200) {
